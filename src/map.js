 ...
 **/
new maplibregl.Marker().setLngLat([-123.0016, 49.2532]).addTo(map);
...
onAuthReady((user) => {
  ...
  if (user) {
    ...
    authButtons.innerHTML = `<button id="logoutBtn" class="btn btn-outline-danger btn-sm">Logout</button>`;
    document.getElementById("logoutBtn").addEventListener("click", async () => {
      await logoutUser(); // logoutUser already redirects to map.html
    });
  } else {
    // Guest — show login button
    authButtons.innerHTML = `<a href="login.html" class="btn btn-primary btn-sm">Login / Sign Up</a>`;
  }
});
